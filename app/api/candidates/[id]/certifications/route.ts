...
import { processCertification } from "@/server/process";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await processCertification(candidateId, file, { skipFileUpload });

    return success(result, 201);
  } catch (err: any) {
    console.error("Failed to process certification:", err);
    ...
    return serverError(err.message || "Failed to process certification");
  }
}
